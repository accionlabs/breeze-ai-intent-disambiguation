  ...
  try {
    ...
    if (autoSharedCount > 0) {
      ...
      result.SHARED_NODES.forEach(nodeId => {
        ...
        if (node) {
          console.log(`  - ${nodeId}: ${node.label} [${node.products?.join(', ') || 'no products'}]`);
        }
      });
    }
    ...
    Object.entries(result.FUNCTIONAL_NODES).forEach(([nodeId, node]: [string, any]) => {
      // Check parents exist
    ...
    console.error(`\n❌ Error testing ${domainName} domain:`, error);
    allDomainsOk = false;
  }
});
